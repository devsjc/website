...
#show: article.with(
  head: "Why I secretly rewrote my company's core infrastructure in my spare time",
  sub: "an introspection on poor work decisions",
  description: "",
  author: "devsjc",
  date: "2026-03-02",
  keywords: ("architecture", "culture", "quality")
)
...
Aside from the ORM, there was at hit to performance arising from the usage of Python. Now as I've said earlier, Python isn't in itself the whole issue: it can be written very well, and the new tooling coming out around it @astral-sh is really helping to normalize quality Python - plus, it's impressively fast for certain applications. I'm also aware that in the world of Machine Learning, there is little viable option other that to use Python - so to a degree, there's no escaping some production Python when ML is part of the product.
