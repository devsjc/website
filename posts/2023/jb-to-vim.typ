...
#show: article.with(
  head: "From JetBrains to Vim",
  sub: "a modern vim configuration and plugin set",
  description: "A walkthrough detailing a Vim setup aimed at emulating the most-used features from JetBrains IDEs.",
  author: "devsjc",
  date: "2023-09-23",
  keywords: ("vim", "vimrc", "python")
)

Short on time? Skip straight to the #link(<vim-configuration>)[configuration]!
...
Recently however, in the wake of the death of it's creator Bram Moolenaar @bram-death,
I have decided to take a look at Vim. As such, I spent a while learning the most basic Vim motions,
and read through Drew Neil's Practical Vim @practical-vim
...
+ Write the function call. The IDE guesses at the function you want to write based on the initial
  characters; you accept it's suggestion and save yourself a few keystrokes.
+ Fill in the parameters of the function. Perhaps you accidentally put a `string` value for a
  parameter that is defined to be an `int`. The IDE underlines your input, informing you of your
  mistake.
+ You right click on the function name in order to jump to the file where it is declared, and see
  that yes, it is expecting an `int`. 
+ You quickly switch back to the original file and modify the call. The IDE tidies up the
...
involvement under the term _Auto Completion_. However, there are a few different processes at play
...
Language Server Protocol (LSP) @lsp. In your IDE, server and client come
...
Step 3 is an example of another *LSP feature*, namely _GoTo Definition_. The language server
...
+ Nice syntax highlighting of any language
+ Git integrations: modified line and file indications in gutter and file browser
+ "Refactor" options to modify all occurrences of a field, function, or class
+ Test pane and test configurations to pass env vars through
+ Terminal pane for testing programs and using command line git
...
Our first remappings! Firstly, the _leader key_ is set to be the space bar - this is up to personal
...
`:botright term` and then `Enter`, or as Vim knows it, _Carriage Return_, hence `<CR>`. The
`nnoremap` means the remap is defined in Vim's _Normal Mode_. See more info on modes
here @vim-modes.

_Update: There is a better workflow for accessing the terminal whilst using Vim, one which simply
...
Plugin: kristijanhusak/vim-packager @vim-packager
...
shorter _buffer_ list via `<leader>b`. The final remap enables a more familiar key combination to
...
_Buffers are a blog post in themselves; I won't go into them here. Suffice to say that newcomers to
vim will find more familiarity in using tabs for their files, but this behaviour should be
discouraged in favour of buffers. This is why I have only created mappings for buffer-based commands._

Have a play with these mappings until their usage becomes familiar.

=== LSP Features: LSP

Plugin: yegappan/lsp @vim-lsp

The first plugin we could point to to back up the "modern" adjective in the title, _lsp_ is written
in and for Vim9, the latest major version of Vim at time of writing. This plugin is going to be
used to perform two of the four features mentioned above; _Auto Completion_, and _LSP Features_
(such as _GoTo Definition_, _Rename_, _Find References_). We incorporate it as follows, again
...
_Language Client_ communicated with a _Language Server_ to get information about a piece of code.
Here we are defining the _language servers_ that the plugin (_the language client_) will use, where
...
lsp wiki @lsp-wiki for the full set and their configurations.
...
the plugin's doc/lsp.txt @lsp-doc file,
accessible in vim with `:h lsp-options`. These again are to preference, but the most important
one here is the first one: `aleSupport: v:true`. This allows us to pass off the _linting_ side
...
Finally, we have more custom remappings. These remap useful _LSP Features_ to the leader key, so
...
This plugin carries out the _Linting_ and _Fixing_ actions mentioned in the background section.
...
again, much more info can be found in the ALE documentation @ale-doc.
...
full list of supported tools @ale-tools.
...
in the plugins' `doc` folder @ale-doc
...
Plugin: bluz71/vim-mistfly-statusline @mistfly
...
vim-devicons @devicons) and the line and column number defining
...
Plugin: vim-test/vim-test @vim-test
...
tpope/vim-dispatch @vim-dispatch plugin for asynchronous running:
...
`<leader>tn` will detect the required test runner and launch it in the _quickfix list_. Whilst this
...
Vim Colour Schemes @vimcolorschemes and see if any call to you. After trying a few I
have settled on sonokai @sonokai:
...
Plugin: sheerun/vim-polyglot @vim-polyglot
...
Plugin: lambdalisue/fern.vim @fern
...
plugins such as preservim/nerdtree @nerdtree for this reason. It's
...
Plugin: liuchengxu/vim-which-key @which-key
...
needed, we can use the _Which Key_ plugin to jog our memory:
...
vimrc, check out this GitHub gist @gist.
...
- Practical Vim, Second Edition by Drew Neil @practical-vim
- Vim Editor Modes Explained (freecodecamp) @vim-modes
