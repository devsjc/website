...
#show: article.with(
  head: "The Complete Guide to pyproject.toml",
  sub: "the simplest way to manage and package python projects",
  description: "A walkthrough detailing a python setup that ditches poetry, setup.py, and even requirements.txt.",
  author: "devsjc",
  date: "2024-06-27",
  keywords: ("pyproject", "python", "packaging", "guide")
)

_Just looking for a `pyproject.toml` to copy to your new project? See the accompanying gist @pyproject-gist!_

== Background: Why do we need pyproject? <background-why-do-we-need-pyproject>

Let's get the elephant in the room out of the way first, because I know what some of you are
thinking: Isn't `pyproject.toml` the poetry @poetry configuration file?
Well, yes it is, but you might not know that it's actually a general python project configuration
file that works with many build frontends and backends @packaging-build-backend,
...
frontend - `pip` - and backend - `setuptools` @setuptools-backend.

#quote(block: true)[
  Although, I'd highly recommend using uv @uv for both! I
...
It has been known for a long time the security risk posed by `setup.py` files @setup-vulnerability -
...
inaccurate reflections of their path when installed as a full package @src-layout.
...
But don't just take my word for it! In case you needed any convincing, the official python.org
packaging guide @python-packaging
...
environment manager: all it needs is `pip`, which comes preinstalled with most python environments @pip-install.
...
create a new virtual environment with your favourite virtual environment tool (most likely `venv` @setuptools-backend)!
...
Since pip doesn't yet have the functionality for automatically modifying `pyproject.toml` files @poetry-add-pep621, these requirements are added in manually, pinning them at the version desired by the developer. Whilst you can specify unversioned dependencies, lets leave
...
=== Editable and normal installations <editable-and-normal-installations>

What's that `-e` flag? This tells pip to perform an _editable_ install, which is the install mode
...
_when working locally, always do an editable install of your package._
...
recommended by `python.org` @src-layout.

For more information on editable installations, see the Setuptools' guide to Development Mode @setuptools-dev-mode.


=== Optional dependencies

The `pyproject.toml` file improves upon `requirements.txt` by allowing the specification of
_optional dependencies_ - dependencies required for parts of local development of the project, but
not integral to running it when distributed. These could be test-
or #link(<configuring-linters>)[linting]-specific requirements, and can be grouped by a key describing
their utility in the `pyproject.toml` file.

For instance, say our project has its test suite written in pytest @pytest,
with a few behave @behave features thrown in. Furthermore, these tests
are running against mock s3 services requiring the moto @moto library.
...
#quote(block: true)[
  ...
  brackets with a backslash, e.g. `pip install -e .\[test\]`! This is because
  `zsh` uses square brackets for pattern matching @zsh-guide.
]
...
#quote(block: true)[
  If you're using `uv`, the command `uv sync` will automatically install the
  `dev` dependency group, so it has a special significance in that context.
  As a result of this, I now just put anything not needed for production in the `dev`
  group and eschew further grouping. `uv` is so fast it feels unecessary to split more!
]
...
I hope you're using it already. If you haven't or aren't, now is a great time to start - it's quick @ruff-formatter,
it's fast @ruff-testimonials, and it's got pace @ruff-pace.
...
ruff docs themselves @ruff-config. It is worth noting that the
...
(agreeing with Linus Torvalds @linus-line-length),
...
some IDE integration (VSCode @ruff-vscode,
JetBrains @ruff-jetbrains, Vim @ale).
...
A blank project is also the best time @mypy-existing
to integrate `mypy`, which bring static type checking to python _a la_ compiled languages. The
...
depth usage instructions, see the mypy documentation @mypy-docs.
Again, there are integrations available for your usual IDEs (VSCode @mypy-vscode,
JetBrains @mypy-jetbrains, Vim @ale).
...
requires the use of a build backend, as mentioned in the #link(<background-why-do-we-need-pyproject>)[Background].
...
attention in the #link(<editable-and-normal-installations>)[editable installations] section, and have
...
package @setuptools-entrypoints:
...
of metadata @writing-pyproject that can
...
(if included; email is an optional field @pyproject-authors),
...
documentation @setuptools-pyproject and the
official python packaging user guide @writing-pyproject.
...
+ `RUN /venv/bin/pip install -q .`: Here we only install the core dependencies in order to keep
...
+ `RUN /venv/bin/pip install .`: Since we didn't build the script for our library earlier, we must
...
+ `FROM gcr.io/distroless/python3-debian11`: In order to improve the security and reduce the size
    of our final container @distroless, we use a
    distroless image, just including the runtime dependencies by copying the virtual environment
    from the `build-app` stage.
+ `ENTRYPOINT ["/venv/bin/coolprojectcli"]`: We leverage the script we specified #link(<entrypoints>)[earlier]
...
most likely that you are personally using GitHub Actions @jb-devecosystem-ci,
...
I know what you're thinking: _We're still running pip install in the test-unit job! What gives?_
...
I know what else you're thinking: _Why did we bother to separate dependencies out if we're going to
build the wheel using a virtual environment with the test dependencies installed?_ Another good
...
`.bumpversion.cfg` or similar files, by using the setuptools-git-versioning @setuptools-git-versioning
package.
...
```sh
$git tag -a v0.2.0 -m "Minor changes for 0.2.0"$ git push --follow-tags
```
