...
#show: article.with(
  head: "[chaser] How to create a bootable Windows USB from MacOS",
  sub: "using nothing but the terminal",
  author: "devsjc",
  date: "2024-11-29",
  keywords: ("sysadmin",)
)
...
Luckily there is a tool, freely available on Apple Silicon via Homebrew @homebrew, that
...
+ Firstly, download the Windows installation media from Microsoft @ms-win10.
  This will be a `.iso` file.
+ Then, get a spare USB stick, greater than 8GB in size. Don't use one with anything you care
  about on it, as it will get wiped in the process! Plug it in. 
+ From your terminal, find the mount point for the USB via
  ```
  $ diskutil list
  ```
...
+ Copy all the files from the mounted ISO, except the `install.wim` file - this is the one that is
...
+ Now it's time to deal with the overly large `install.wim` file. This is where the `wimlib` tool
...
+ Finally, unmount the ISO and the USB via
  ```
