...
#show: article.with(
  head: "Database Connections in the Terminal",
  sub: "Leveraging SSH and Vim to inspect remote databases",
  description: "A guide to setting up a database IDE in Vim, with a focus on managing SSH tunnels.",
  author: "devsjc",
  date: "2024-04-05",
  keywords: ("vim", "ssh", "sql")
)

== Background: What's the Aim?

As a developer, you’ll inevitably find yourself having to interact with databases. There are
GUI tools available (DataGrip @datagrip, DBeaver @dbeaver),
but in the spirit of my previous article @prev-article
...
vim-dadbod @vim-dadbod and vim-dadbod-ui @vim-dadbod-ui,
and can be installed with your favourite plugin manager. Alongside these, we’ll glean some extra
useful functionality with another plugin: vim-dispatch @vim-dispatch.
We’ll come on to this one later.

Assuming a setup as described in "From Jetbrains to Vim" @prev-article,
...
straightforward to the local case (see `ADAPTERS` heading of `vim-dadbod`’s help @vim-dadbod-help).
...
half an eye on it for the roadmap of the `vim-dadbod-ui` plugin @vim-dadbod-ui-issue.
...
+ Launch Vim: `$ vim`
+ Asynchronously spin up a database connection, using the SSH config alias: `:Start ssh -N -v bastion`
+ Launch DBUI, which reads the connections defined earlier from the JSON file: `:DBUI`
